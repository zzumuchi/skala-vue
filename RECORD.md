...
#### 지역 날씨 지도 및 라이프 추천 확장

- 기존 서울, 수원, 부산, 판교에 강릉과 제주를 추가해 주요 여섯 지역의 실시간 날씨를 지도에서 비교
- 지역 ID, 한글 지역명, API 좌표와 지도 지점 위치를 `weatherCities.js`에서 공통 관리
- 대한민국 형태의 CSS 기반 반응형 지도와 클릭 가능한 지역 지점을 `WeatherMap.vue`로 분리
- 지도 지점 선택 시 추가 API 요청 없이 기존 현재 날씨 응답을 재사용해 기온, 상태, 습도와 풍속 표시
- OpenWeather 기상 코드, 기온과 풍속을 조합하는 `weatherRecommendation.js` 추천 규칙 작성
- 비, 눈, 뇌우, 안개, 폭염, 한파와 강풍 조건에 따라 추천 활동, 옷차림과 외출 팁을 동적으로 제공
- 추천 패널에서 선택 지역의 상세 예보와 대기질 화면으로 이동하도록 Router 연결
- 홈 카드와 상세 화면의 정적 추천 문구를 실시간 날씨 기반 추천 결과로 변경
- 여러 지역 중 일부 API 요청이 실패해도 성공한 지역은 유지하도록 `Promise.allSettled()` 적용
- 넓어진 지도 화면에 맞춰 앱 최대 너비를 확장하고 모바일에서는 지도와 추천 패널을 한 열로 배치
- 문서 언어를 한국어로 지정하고 브라우저 제목을 `지역 날씨 라이프 가이드`로 변경
